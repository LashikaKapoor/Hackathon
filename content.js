...
function collectCandidateBlocks() {
  const nodes = [...document.querySelectorAll("article,[role='listitem'],a[href*='/a/'],a[href*='/c/']")];
  const seen = new Set();
  const blocks = [];
...
function getItemType(text, link) {
  if (/\/a\//.test(link || "") || /assignment|homework|turned in|missing|due/i.test(text)) return "Assignment";
  return "Stream post";
}
